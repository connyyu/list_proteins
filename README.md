...
This repository contains a Python script that retrieves and lists all the protein chains and bound ligands in PDB structural files.

The tool downloads updated mmCIF files from PDBe, parse the files and lists the UniProt accessions and names (UniProt ID) of each protein chain. It generates a PyMOL script to label each protein chain by creating a selection with its UniProt ID and colouring each chain for visualisation.
